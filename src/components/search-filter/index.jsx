...
const SearchFilter = () => {
  ...
  const [filterValues, setFilterValues] = useState({price: '{"low": 50, "high" : 350}', exp: experiences, language: languages})
  ...
  return(
    <StyledSearchFilter > 
      <Select  placeholder='Preço' onChange={handleFilterPrice} className='filter-price'>
        {prices.map((price, index) => <Option key={index} value={price.value}>{price.title}</Option>)}
        </Select>

        <Select  mode='multiple' placeholder='Experiência' className='filter-experience' onChange={handleFilterExperience}>
          {experiences.map((exp, index) => <Option key={index} value={exp}>{exp}</Option>)}
        </Select>
        <Select  placeholder='Idioma' onChange={handleLanguage} className='filter-language'>
